...
const guard = path.join(root, 'scripts', 'release', 'check-k4c-skills-only.mjs');
const packageRoot = path.join(root, 'packages', 'codex', 'kaleidosphere');
const canonicalRoot = path.join(root, 'agent-skills', 'kaleidosphere');
const policy = path.join(root, 'docs', 'release', 'k4c-skills-only-policy-v1.json');
const fixture = path.join(root, 'tests', 'fixtures', 'release', 'k4c-prohibited-package-inputs-v1.json');
...
async function manifestPath(packageDir) {
  return path.join(packageDir, '.codex-plugin', 'plugin.json');
}
...
test('positive: canonical skill bytes plus the matching Apache-2.0 manifest pass', async () => {
  const packageDir = await packageCopy('ks76-static-positive-');
...
test('negative fixture coverage: secret-like values are denied without echoing contents', async () => {
  const packageDir = await packageCopy('ks76-static-secret-');
...
test('negative fixture coverage: executable payloads and symlink escapes are denied', async () => {
  const executable = await packageCopy('ks76-static-executable-');
  ...
  const escaped = await packageCopy('ks76-static-symlink-');
...
test('negative fixture coverage: a license mismatch is denied before digest acceptance', async () => {
  const packageDir = await packageCopy('ks76-static-license-');
