...
const validatorAdapter = path.join(root, 'scripts', 'release', 'validate-k4c-codex-plugin.mjs');
const validatorTranscript = path.join(root, 'tests', 'fixtures', 'release', 'k4c-plugin-creator-transcripts-v1.json');
...
test('generated plugin manifest validates through the portable plugin-creator adapter', async () => {
  const { dir } = await build('ks76-k4c-validator-');
  const result = run(process.execPath, [validatorAdapter, '--fixture', validatorTranscript, '--out', dir, '--dry-run'], { cwd: root });
