...
async function variant(mutate, prefix = 'ks76-submission-kit-') {
...
test('rejects unsupported listing claims', async () => {
  const directory = await mkdtemp(path.join(tmpdir(), 'ks76-submission-listing-'));
  const listing = path.join(directory, 'listing.md');
  const original = await readFile(path.join(root, 'docs/release/k4c-directory-listing.md'), 'utf8');
