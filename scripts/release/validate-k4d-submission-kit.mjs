#!/usr/bin/env node
// Validate the K4D Claude Code directory submission packet without submitting anything.
// The packet is accepted only when its copy and reviewer matrix are bound to the
// already-recorded local package receipts. No marketplace submission, publication,
// or portal mutation is performed; the packet is a non-executable evidence bundle.
...
const defaultListing = path.join(root, 'docs', 'release', 'k4d-directory-listing.md');
const defaultReleaseNotes = path.join(root, 'docs', 'release', 'k4d-release-notes.md');
const defaultFixture = path.join(root, 'tests', 'fixtures', 'release', 'k4d-reviewer-test-cases-v1.json');
const securityReceipt = path.join(root, 'generated', 'claude', 'receipts', 'skills-only-security-license-receipt-v1.json');
const e2eReceipt = path.join(root, 'generated', 'claude', 'receipts', 'claude-isolated-e2e-v1.json');
const manifest = path.join(root, 'generated', 'claude', 'kaleidosphere', '.claude-plugin', 'plugin.json');
const SCHEMA = 'kaleidosphere/k4d-reviewer-test-cases/v1';
const SECURITY_SCHEMA = 'kaleidosphere/k4d-skills-only-security-license-receipt/v1';
const E2E_SCHEMA = 'kaleidosphere/k4d-claude-isolated-e2e/v1';
const PACKAGE_DIGEST = 'a9dfd0e40633c13ab7b04f15bdbfd8d5fa579453717272a9542a87567b13a255';
const MANIFEST_DIGEST = 'b8a53a99c90b10982ca7cd15291d000291dc6a0e511b6b6ff53b2222741ae42d';
...
function assertReceiptBindings(security, e2e, manifestBytes) {
  ...
  if (security.schemaVersion !== SECURITY_SCHEMA) throw new Error('security receipt schema drift denied');
  if (e2e.schemaVersion !== E2E_SCHEMA) throw new Error('isolated E2E receipt schema drift denied');
...
function assertListing(listing) {
  if (!listing.includes('# KaleidoSphere Claude Code directory listing') || !listing.includes('## Claims')) {
...
function assertReleaseNotes(notes) {
  ...
  for (const value of [VERSION, PACKAGE_DIGEST, MANIFEST_DIGEST, 'generated/claude/receipts/skills-only-security-license-receipt-v1.json', 'generated/claude/receipts/claude-isolated-e2e-v1.json']) {
    if (!notes.includes(value)) throw new Error(`release notes evidence binding missing: ${value}`);
  }
  if (!/No marketplace submission has been performed\./.test(notes)) throw new Error('release notes marketplace boundary missing');
}
...
function assertCaseMatrix(fixture) {
  ...
  if (!byId.get('prohibited-package-content').evidence.some((item) => item.includes('check-k4d-skills-only') || item.includes('prohibited-package-inputs'))) {
...
function buildResult(args, fixture, overrides = {}) {
  return {
    ...
    listing: args.listing === defaultListing ? 'docs/release/k4d-directory-listing.md' : args.listing,
    releaseNotes: args.releaseNotes === defaultReleaseNotes ? 'docs/release/k4d-release-notes.md' : args.releaseNotes,
...
try {
  ...
  process.stderr.write(`k4d-submission-kit: ${error.message}\n`);
  process.exitCode = 1;
}