#!/usr/bin/env node
// Deterministic K4D Claude Code skills-only plugin generator (KaleidoSphere, KS077).
//
// Builds the skills-only generated package view `generated/claude/kaleidosphere`
// whose `skills/kaleidosphere` tree is an exact copied view of the canonical
// `agent-skills/kaleidosphere` skill, plus the frozen `.claude-plugin/plugin.json`
// manifest. The manifest is a thin, official Claude Code plugin schema surface
// that declares ONLY skills (no commands, agents, hooks, MCP, LSP, or settings).
// Every run prints a digest receipt recording the canonical/generated sha256 pair
// for every copied file and a stable package digest. The receipt is deterministic:
// no timestamps, no random values.
//
// Usage:
//   node scripts/release/generate-k4d-claude-plugin.mjs
//   node scripts/release/generate-k4d-claude-plugin.mjs --check --fixture tests/fixtures/release/k4d-canonical-skill-digests-v1.json
...
const PACKAGE_REL = 'generated/claude/kaleidosphere';
...
const RECEIPT_SCHEMA = 'kaleidosphere/k4d-claude-plugin-receipt/v1';
const FIXTURE_SCHEMA = 'kaleidosphere/k4d-canonical-skill-digests/v1';
const PACKAGE_DIGEST_SCHEMA = 'kaleidosphere/k4d-claude-plugin-package-digest/v1';
const FORBIDDEN_SURFACES = ['commands', 'agents', 'hooks', 'mcpServers', 'lspServers', 'settings'];
const FROZEN_MANIFEST = {
  version: '0.26.0',
  sha256: 'b8a53a99c90b10982ca7cd15291d000291dc6a0e511b6b6ff53b2222741ae42d',
};
...
const activeHostPath = /(?:^|\/)(?:hooks?(?:\.json)?|mcp(?:-?servers?)?(?:\.json)?|lsp(?:-?servers?)?(?:\.json)?)(?:\/|$)/i;
const executableArtifact = /\.(?:sh|bash|zsh|fish|exe|dll|so|dylib|wasm|node|py)$/i;
...
function buildManifestBytes(packageVersion) {
  const manifest = {
    ...
    license: 'Apache-2.0',
    keywords: ['kaleidosphere', 'agent-skill', 'business-intelligence'],
    skills: './skills/',
  };
...
async function loadCanonicalRecords(canonicalDir) {
  ...
  for (const relative of relatives) {
    if (activeHostPath.test(relative)) throw new Error(`hook, MCP or LSP path denied in canonical skill: ${relative}`);
...
function assertManifestSurfaces(plugin) {
  for (const key of FORBIDDEN_SURFACES) {
    if (key in plugin) throw new Error(`active plugin surface denied: ${key}`);
  }
  if (plugin.skills !== './skills/') throw new Error('plugin manifest skills path drift denied');
}
...
try {
  ...
  process.stderr.write(`k4d-claude-plugin: ${error.message}\n`);
  process.exitCode = 1;
}