#!/usr/bin/env node
// Fail-closed static security guard for the generated K4D Claude Code package.
// This guard only reads package inputs and writes an optional local receipt. It
// deliberately does not invoke the repository generator, the `claude` runtime,
// or any other external command; generator and official `claude plugin validate`
// acceptance remain separate gates recorded by validate-k4d-claude-plugin.mjs.
...
const defaultPackage = path.join(root, 'generated', 'claude', 'kaleidosphere');
const defaultCanonical = path.join(root, 'agent-skills', 'kaleidosphere');
const defaultPolicy = path.join(root, 'docs', 'release', 'k4d-skills-only-policy-v1.json');
const defaultFixture = path.join(root, 'tests', 'fixtures', 'release', 'k4d-prohibited-package-inputs-v1.json');
const defaultReceipt = path.join(root, 'generated', 'claude', 'receipts', 'skills-only-security-license-receipt-v1.json');
const POLICY_SCHEMA = 'kaleidosphere/k4d-skills-only-policy/v1';
const FIXTURE_SCHEMA = 'kaleidosphere/k4d-prohibited-package-inputs/v1';
const RECEIPT_SCHEMA = 'kaleidosphere/k4d-skills-only-security-license-receipt/v1';
const PACKAGE_DIGEST_SCHEMA = 'kaleidosphere/k4d-claude-plugin-package-digest/v1';
const CANONICAL_DIGEST_FIXTURE_SCHEMA = 'kaleidosphere/k4d-canonical-skill-digests/v1';
...
function assertFixture(fixture, policy) {
  ...
  if (fixture.policy !== 'docs/release/k4d-skills-only-policy-v1.json') throw new Error('prohibited package input fixture policy binding denied');
...
async function loadCanonical(canonicalDir, policy, fixturePath) {
  ...
  if (fixture.schemaVersion !== CANONICAL_DIGEST_FIXTURE_SCHEMA || fixture.canonicalSource !== policy.canonical.source) {
...
function buildReceipt(packageDir, canonicalDir, policy, fixturePath, canonical, inspected) {
  ...
  const receipt = {
    ...
    policy: 'docs/release/k4d-skills-only-policy-v1.json',
    fixture: 'tests/fixtures/release/k4d-prohibited-package-inputs-v1.json',
    ...
    scannedPaths: {
      ...
      inputs: [
        'docs/release/k4d-skills-only-policy-v1.json',
        'tests/fixtures/release/k4d-prohibited-package-inputs-v1.json',
...
try {
  ...
  process.stderr.write(`k4d-skills-only: ${error.message}\n`);
  process.exitCode = 1;
}